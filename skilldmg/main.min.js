...
b=calcSkillDamage(c.value,parseInt(d),document.getElementById("dshift").value,parseFloat(e),b);document.getElementById("totaldmg").innerText=`${b.min} ~ ${b.max}`;"pois"==c.value?(d=((parseFloat(b.min)+parseFloat(b.max))/2/(b.len/25)).toFixed(2),document.getElementById("dmgpersec").innerText=`${isNaN(d)?0:d} /s`):document.getElementById("dmgpersec").innerText="0 /s";document.getElementById("totaldmglen").innerText=`${b.len/25} /s`;switch(c.value){case "phys":document.getElementById("totaldmg").setAttribute("class",
